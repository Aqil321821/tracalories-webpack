...
class App {
    ...
    _removeItem(type, e) {
        if (e.target.classList.contains('delete') || (e.target.classList.contains('fa-xmark'))) {
            if (confirm('Are You Sure To Delete')) {
                ...
                e.target.closest('.card').remove(); // *** Impt Note //
    ...
    _filterItems(type, e) {
        ...
        document.querySelectorAll(`#${type}-items .card`).forEach((item) => {
            ...
            if (name.toLowerCase(), indexOf(text) !== -1) {
                ...
                //if both matches then display else not display
    ...
    _reset() {
        //clear all values of calories
        this._tracker.reset();
        //clear all dom elemnts which we can insert 
